...
class AdvertisingInfoServiceStrategy implements AdvertisingInfoStrategy {
    ...
    private static final class AdvertisingConnection implements ServiceConnection {
        ...
        public IBinder getBinder() {
            if (this.retrieved) {
                Fabric.getLogger().mo3690e(Fabric.TAG, "getBinder already called");
            }
            this.retrieved = true;
            try {
    ...
    private static final class AdvertisingInterface implements IInterface {
        ...
        public String getId() throws RemoteException {
            ...
            try {
                ...
                Fabric.getLogger().mo3688d(Fabric.TAG, "Could not get parcel from Google Play Service to capture AdvertisingId");
            } finally {
                reply.recycle();
                data.recycle();
            }
        ...
        L_0x002f:
            ...
            r4.mo3688d(r5, r6);	 Catch:{ all -> 0x0044 }
            r3.recycle();
            r0.recycle();
            goto L_0x002c;
        L_0x0044:
    ...
    public AdvertisingInfo getAdvertisingInfo() {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            Fabric.getLogger().mo3688d(Fabric.TAG, "AdvertisingInfoServiceStrategy cannot be called on the main thread");
            return null;
        }
        try {
            ...
            try {
                ...
                Fabric.getLogger().mo3688d(Fabric.TAG, "Could not bind to Google Play Service to capture AdvertisingId");
                return null;
            } catch (Exception e) {
                Fabric.getLogger().mo3702w(Fabric.TAG, "Exception in binding to Google Play Service to capture AdvertisingId", e);
                this.context.unbindService(connection);
                return null;
            } catch (Throwable t) {
                Fabric.getLogger().mo3689d(Fabric.TAG, "Could not bind to Google Play Service to capture AdvertisingId", t);
                return null;
            }
        } catch (NameNotFoundException e2) {
            Fabric.getLogger().mo3688d(Fabric.TAG, "Unable to find Google Play Services package name");
            return null;
        } catch (Exception e3) {
            Fabric.getLogger().mo3689d(Fabric.TAG, "Unable to determine if Google Play Services is available", e3);
            return null;
        }
    }
}
