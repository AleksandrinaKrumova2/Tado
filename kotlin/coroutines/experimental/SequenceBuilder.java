...
public abstract class SequenceBuilder<T> {
    ...
    @Nullable
    public final Object yieldAll(@NotNull Iterable<? extends T> elements, @NotNull Continuation<? super Unit> continuation) {
        if ((elements instanceof Collection) && ((Collection) elements).isEmpty()) {
            return Unit.INSTANCE;
        }
        return yieldAll(elements.iterator(), (Continuation) continuation);
    }

    @Nullable
    public final Object yieldAll(@NotNull Sequence<? extends T> sequence, @NotNull Continuation<? super Unit> continuation) {
        return yieldAll(sequence.iterator(), (Continuation) continuation);
    }
}
