...
public class ZoneInfoCompiler {
    ...
    public Map<String, DateTimeZone> compile(File file, File[] fileArr) throws IOException {
        ...
        for (int i3 = 0; i3 < this.iZones.size(); i3++) {
            ...
            DateTimeZone toDateTimeZone2 = dateTimeZoneBuilder.toDateTimeZone(zone.iName, true);
            if (test(toDateTimeZone2.getID(), toDateTimeZone2)) {
                treeMap.put(toDateTimeZone2.getID(), toDateTimeZone2);
                treeMap2.put(toDateTimeZone2.getID(), zone);
                if (file != null) {
                    writeZone(file, dateTimeZoneBuilder, toDateTimeZone2);
                }
            }
        }
        for (i = 0; i < this.iGoodLinks.size(); i += 2) {
            ...
            if (zone2 == null) {
                ...
                toDateTimeZone = dateTimeZoneBuilder2.toDateTimeZone(str2, true);
                if (test(toDateTimeZone.getID(), toDateTimeZone)) {
                    treeMap.put(toDateTimeZone.getID(), toDateTimeZone);
                    if (file != null) {
                        writeZone(file, dateTimeZoneBuilder2, toDateTimeZone);
                    }
                }
                treeMap.put(toDateTimeZone.getID(), toDateTimeZone);
        ...
        for (int i4 = 0; i4 < 2; i4++) {
            for (i = 0; i < this.iBackLinks.size(); i += 2) {
                ...
                toDateTimeZone = (DateTimeZone) treeMap.get(str);
                if (toDateTimeZone != null) {
                    treeMap.put(str2, toDateTimeZone);
                    if (ZoneInfoLogger.verbose()) {
                        System.out.println("Back link: " + str2 + " -> " + toDateTimeZone.getID());
                    }
                } else if (i4 > 0) {
