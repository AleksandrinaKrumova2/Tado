...
final class RealBufferedSource implements BufferedSource {
    ...
    public String readUtf8LineStrict(long limit) throws IOException {
        ...
        if (scanLength < LongCompanionObject.MAX_VALUE && request(scanLength) && this.buffer.getByte(scanLength - 1) == (byte) 13 && request(1 + scanLength) && this.buffer.getByte(scanLength) == (byte) 10) {
            return this.buffer.readUtf8Line(scanLength);
        }
        Buffer data = new Buffer();
        this.buffer.copyTo(data, 0, Math.min(32, this.buffer.size()));
        throw new EOFException("\\n not found: limit=" + Math.min(this.buffer.size(), limit) + " content=" + data.readByteString().hex() + Typography.ellipsis);
    }
    ...
    public int readUtf8CodePoint() throws IOException {
        ...
        if ((b0 & 224) == 192) {
            require(2);
        } else if ((b0 & DimensionsKt.HDPI) == 224) {
            require(3);
        } else if ((b0 & 248) == DimensionsKt.HDPI) {
            require(4);
        }
