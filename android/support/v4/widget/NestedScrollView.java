...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
    ...
    static class SavedState extends BaseSavedState {
        public static final Creator<SavedState> CREATOR = new C03281();
        public int scrollPosition;

        static class C03281 implements Creator<SavedState> {
            C03281() {
            }
        ...
        if (canScroll()) {
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    ...
                    case 20:
                        if (!event.isAltPressed()) {
                            handled = arrowScroll(Constants.DEFAULT_HOT_WATER_TEMPERATURE_FAHRENHEIT);
                            break;
                        }
                        handled = fullScroll(Constants.DEFAULT_HOT_WATER_TEMPERATURE_FAHRENHEIT);
                        break;
                    case 62:
                        pageScroll(event.isShiftPressed() ? 33 : Constants.DEFAULT_HOT_WATER_TEMPERATURE_FAHRENHEIT);
                        break;
                }
            }
            return handled;
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, Constants.DEFAULT_HOT_WATER_TEMPERATURE_FAHRENHEIT);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(Constants.DEFAULT_HOT_WATER_TEMPERATURE_FAHRENHEIT)) {
                return false;
            }
            return true;
        }
    ...
    public boolean pageScroll(int direction) {
        boolean down;
        if (direction == Constants.DEFAULT_HOT_WATER_TEMPERATURE_FAHRENHEIT) {
            down = true;
        } else {
            down = false;
        }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (nextFocused == null || !isWithinDeltaOfScreen(nextFocused, maxJump, getHeight())) {
            ...
            } else if (direction == Constants.DEFAULT_HOT_WATER_TEMPERATURE_FAHRENHEIT && getChildCount() > 0) {
    ...
    protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = Constants.DEFAULT_HOT_WATER_TEMPERATURE_FAHRENHEIT;
        } else if (direction == 1) {
            direction = 33;
        }
