...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C03941();
            ...
            class C03941 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C03951.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C03961();
            ...
            class C03961 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C03972.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C03972.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C03972.this.mQueue.removeMessages(2);
                                    C03972.this.mQueue.removeMessages(3);
                        ...
                        C03972.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
