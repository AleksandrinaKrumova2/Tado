...
public final class BridgeInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        Builder responseBuilder = networkResponse.newBuilder().request(userRequest);
        if (transparentGzip && HttpRequest.ENCODING_GZIP.equalsIgnoreCase(networkResponse.header(HttpRequest.HEADER_CONTENT_ENCODING)) && HttpHeaders.hasBody(networkResponse)) {
            ...
            responseBuilder.headers(networkResponse.headers().newBuilder().removeAll(HttpRequest.HEADER_CONTENT_ENCODING).removeAll(HttpRequest.HEADER_CONTENT_LENGTH).build());
            responseBuilder.body(new RealResponseBody(networkResponse.header(HttpRequest.HEADER_CONTENT_TYPE), -1, Okio.buffer(responseBody)));
        }
