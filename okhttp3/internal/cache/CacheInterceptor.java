...
public final class CacheInterceptor implements Interceptor {
    ...
    private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
        ...
        return response.newBuilder().body(new RealResponseBody(response.header(HttpRequest.HEADER_CONTENT_TYPE), response.body().contentLength(), Okio.buffer(cacheWritingSource))).build();
    }
    ...
    private static Headers combine(Headers cachedHeaders, Headers networkHeaders) {
        ...
        for (i = 0; i < size; i++) {
            ...
            if (!HttpRequest.HEADER_CONTENT_LENGTH.equalsIgnoreCase(fieldName) && isEndToEnd(fieldName)) {
