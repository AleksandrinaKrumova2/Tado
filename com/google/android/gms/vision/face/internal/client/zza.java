...
public final class zza extends zzdjv<zze> {
    ...
    private static Landmark[] zza(FaceParcel faceParcel) {
        ...
        while (i < landmarkParcelArr.length) {
            ...
            landmarkArr[i] = new Landmark(new PointF(landmarkParcel.f11x, landmarkParcel.f12y), landmarkParcel.type);
            i++;
        }
        return landmarkArr;
    }
    ...
    public final boolean zzez(int i) {
        if (!isOperational()) {
            return false;
        }
        try {
            return ((zze) zzbjv()).zzez(i);
        } catch (Throwable e) {
            Log.e("FaceNativeHandle", "Could not call native face detector", e);
            return false;
        }
