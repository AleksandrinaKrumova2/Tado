...
public class SimpleMonthView extends MonthView {
    ...
    public void drawMonthDay(Canvas canvas, int year, int month, int day, int x, int y, int startX, int stopX, int startY, int stopY) {
        ...
        if (!isHighlighted(year, month, day) || this.mSelectedDay == day) {
            this.mMonthNumPaint.setTypeface(Typeface.create(Typeface.DEFAULT, 0));
        } else {
            canvas.drawCircle((float) x, (float) ((MINI_DAY_NUMBER_TEXT_SIZE + y) - DAY_HIGHLIGHT_CIRCLE_MARGIN), (float) DAY_HIGHLIGHT_CIRCLE_SIZE, this.mSelectedCirclePaint);
        ...
        canvas.drawText(String.valueOf(day), (float) x, (float) y, this.mMonthNumPaint);
    }
}
